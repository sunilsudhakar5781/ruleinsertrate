...
void check_entry_status(struct doca_flow_pipe_entry *entry,
			uint16_t pipe_queue,
			       ...
			       int nb_queue_depth)
{
	...
	result = doca_flow_cfg_set_queue_depth(flow_cfg, nb_queue_depth);
	if (result != DOCA_SUCCESS) {
		DOCA_LOG_ERR("Failed to set queue_depth of 1024: %s", doca_error_get_descr(result));
		goto destroy_cfg;
	}


	for (i = 0; i < SHARED_RESOURCE_NUM_VALUES; i++) {
				...
				uint32_t actions_mem_size[])
{
	...
	for (portid = 0; portid < nb_ports; portid++) {
		...
		result =
			create_doca_flow_port(portid, dev_arr[portid], state,
					      &ports[portid], actions_mem_size[portid]);
		if (result != DOCA_SUCCESS) {
	...
	/* Pair ports should be done in the following order:
	 * port0 with port1, port2 with port3 etc
	 */
...
doca_error_t init_doca_flow_ports(int nb_ports, struct doca_flow_port *ports[],
				  ...
				  uint32_t actions_mem_size[])
{
	return init_doca_flow_ports_with_op_state(nb_ports, ports, is_hairpin,
						  dev_arr, NULL, actions_mem_size);
}
